    ...
    @foreach ( $items_arr as $sclass_id =>  $items)
        <tr data-id="{{$sclass_id}}" class="sclasse sclasse_{{$sclass_id}}" style="background-color: #ddd;">
            <td class="lighter-color">
                <i 
                data-item_id="{{$sclass_id}}" data-type="sclass" 
                class="addFavorite curson-poiter favoriteScheduleSclass favoriteScheduleSclass_{{$sclass_id}} far fa-star {{ in_array($sclass_id,$favorites['sclass']) ? 'fas' : '' }}"></i>
            </td>
        ...
        <tr data-id="{{$item->schedule_id}}" class="schedule schedule_{{ $item->schedule_id }} belongTo_sclasse_{{$sclass_id}}">
            <td class="lighter-color">
                <i data-item_id="{{$item->schedule_id}}" data-type="match" 
                class="addFavorite curson-poiter favoriteScheduleMatch favoriteScheduleMatch_{{$item->schedule_id}} far fa-star  {{ in_array($item->schedule_id,$favorites['match']) ? 'fas' : '' }}"></i></td>
